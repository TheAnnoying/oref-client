<script lang="ts">
    import { alert, localStorage, preferredLocations, muted } from "$lib/index.ts";
	...
	onMount(async() => {
		...
		alert.subscribe(() => {
			...
			const shouldPlaySound = !$alert?.type?.endsWith("הסתיים") && !get(muted) && keyAmount > 0 && ($preferredLocations.length === 0 ? true : $preferredLocations.some(e => $alert?.locations?.includes(e[0])));
...
{#if Object.keys($alert).length > 0}
	<div transition:fade={{ duration: 350 }}>
		<Card.Root class="my-4 border-destructive border-t-destructive! border-b-destructive! shadow-[0_0_5px] shadow-destructive sm:w-[600px] w-[350px] transition-transform">
			<Card.Header>
				<Card.Title class="text-xl flex flex-row items-center gap-2">
					<Radio />
					התרעה כרגע
